...
class byCategory(ListView):
    ...
    def get(self, request, *args, **kwargs) :
        if self.request.GET:
            ...
            post = Paginator(category, 4)
            if(self.request.GET.get("page")):
                page_obj = post.page(self.request.GET.get("page"))  
            else:
                page_obj = post.page(1)
            context={"entries":page_obj,"text":"Nuestros productos","tag":2,"tag2":kwargs["slug"]}
    ...
    def getCategory(self,id,slug):
        ...
        if(self.request.GET.get("page")):
            ...
            page_obj = post.page(1)
        return {"entries":page_obj,"tag":1,"tag2":slug}
...
class byMarcas(TemplateView):
    ...
    def get(self,request,*args, **kwargs):
        ...
        if(request.GET.get("page")):
            ...
            page_obj = post.page(1)
        return render(request,self.template_name,{"entries":page_obj ,"tag":2,"tag2":kwargs['marca'],"productActive":"active","displayBrand":"none"})
